...
FRESULT f_getfree (
	const TCHAR* path,	/* Path name of the logical drive number 存储介质标号 */
	DWORD* nclst,		/* Pointer to a variable to return number of free clusters 返回空白扇区大小 */
	FATFS** fatfs		/* Pointer to return pointer to corresponding file system object 返回一个指针(句柄)指向文件 */
)
{
	FRESULT res;
	FATFS *fs;
