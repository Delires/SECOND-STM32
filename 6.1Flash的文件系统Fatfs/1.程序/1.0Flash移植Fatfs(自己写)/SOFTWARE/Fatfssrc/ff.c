...
/----------------------------------------------------------------------------*/
/** 核心的FatFs文件系统管理的相关函数 f_open f_read等)                       **/
...
FRESULT f_mount (
	FATFS* fs,			/* Pointer to the file system object (NULL:unmount) 文件系统的指针(指针为空，取消挂载)*/
	const TCHAR* path,	/* Logical drive number to be mounted/unmounted 存储介质编号*/
	BYTE opt			/* 0:Do not mount (delayed mount), 1:Mount immediately  功能选项  一般都是1立即挂载*/
)
{
	FATFS *cfs;
	int vol;
...
const TCHAR* path,	/* Pointer to the file name  存储介质标号＋文件名 "1:xxx" */
...
FRESULT f_read (
	FIL* fp, 		/* Pointer to the file object 文件句柄*/
	void* buff,		/* Pointer to data buffer    读到哪个数组里*/
	UINT btr,		/* Number of bytes to read   要读的数组的个数  用一个f_size()函数直接检测文件大小*/
	UINT* br		/* Pointer to number of bytes read  函数返回时此处读完，已经读了多少个数据 */
)
{
	FRESULT res;
...
FRESULT f_write (
	FIL* fp,			/* Pointer to the file object  文件句柄*/
	const void *buff,	/* Pointer to the data to be written  写入值*/
	UINT btw,			/* Number of bytes to write   写入的文件大小*/
	UINT* bw			/* Pointer to number of bytes written  函数返回时此次写入多少个字节值*/
)
{
	FRESULT res;
	DWORD clst, sect;
...
/**
f_opendir 打开一个文件夹目录 opens a directory
**/
FRESULT f_opendir (
	DIR* dp,			/* Pointer to directory object to create  文件夹句柄*/
const TCHAR* path	/* Pointer to the directory path 这个目录路径可以直接是"1:"  也可以是 "1:xxx" 看你扫描哪呢 只要是文件夹名都行*/
)
{
	FRESULT res;
	FATFS* fs;
...
/*-----------------------------------------------------------------------*/
/* Read Directory Entries in Sequence  读取文件目录里的项目 读文件夹里的文件                                  */
/*-----------------------------------------------------------------------*/

FRESULT f_readdir (
	DIR* dp,			/* Pointer to the open directory object 文件句柄 */
	FILINFO* fno		/* Pointer to file information to return 返回值存储的结构体*/
)
{
	FRESULT res;
	DEFINE_NAMEBUF;
...
FRESULT f_stat (
	const TCHAR* path,	/* Pointer to the file path 存储器中的文件名*/
	FILINFO* fno		/* Pointer to file information to return 返回值得结构体变量*/
)
{
	FRESULT res;
	DIR dj;
...
FRESULT f_unlink (
	const TCHAR* path		/* Pointer to the file or directory path  输入文件名 删除文件  例如："1:mengdi.txt" 或"1:DirText/ceshi.txt" 用/层层到子文件 */
)
{
	FRESULT res;
	DIR dj, sdj;
...
/*-----------------------------------------------------------------------*/
/* Create a Directory    建立一个文件夹                                                 */
/*-----------------------------------------------------------------------*/

FRESULT f_mkdir (
	const TCHAR* path		/* Pointer to the directory path “1：xxx” */
)
{
	FRESULT res;
	DIR dj;
...
FRESULT f_rename (
	const TCHAR* path_old,	/* Pointer to the object to be renamed 老文件夹名字 */
	const TCHAR* path_new	/* Pointer to the new name 新文件夹名字  文件夹/文件名 带/的操作可以移动到文件夹里 */
)
{
	FRESULT res;
	DIR djo, djn;
...
/*-----------------------------------------------------------------------*/
/* Create file system on the logical drive  产生一个文件系统 并且初始化                               */
/*-----------------------------------------------------------------------*/
...
FRESULT f_mkfs (
	const TCHAR* path,	/* Logical drive number 存储介质号*/
	BYTE sfd,			/* Partitioning rule 0:FDISK, 1:SFD FDISK硬盘 SFD软盘 一般此参数为0*/
	UINT au				/* Size of allocation unit in unit of byte or sector 每个簇的大小=多少扇区  一般配置为0 */
)
{
