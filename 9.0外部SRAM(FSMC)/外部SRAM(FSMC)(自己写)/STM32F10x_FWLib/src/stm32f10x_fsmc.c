...
{ 
  ...
  FSMC_Bank1->BTCR[FSMC_NORSRAMInitStruct->FSMC_Bank] =  //FSMC_Bank1被定义为一个指针, 则FSMC_Bank1->BTCR[FSMC_NORSRAMInitStruct->FSMC_Bank] 就是 FSMC_Bank1结构体里的 BTCR[]的指针值
            (uint32_t)FSMC_NORSRAMInitStruct->FSMC_DataAddressMux |    //寄存器地址值0xA000 0000 + 8 * (x-1), x=1…4
            ...
            FSMC_NORSRAMInitStruct->FSMC_WriteBurst;       //结构体的数据 给向一个指针值
