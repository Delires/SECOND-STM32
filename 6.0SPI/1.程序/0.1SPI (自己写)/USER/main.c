...
/***************  写数据 **************************************/
		...
		/*** 读取FLSH ***/ 
		Read_Sector_Flash(0x000015, ReadData, 500);   //1k = 1024byte (不是位)
		printf("\r\n Flash读取的数据为\r\n"); 
...
/***************  写字符串 **************************************/
		...
		printf("\r\n 往Flash存储的的数据为:\r\n%s\r\n", WriteData_char );  //打印字符串 直接输数组名
     	Write_Less256byte_Flash(0x000000, WriteData_char, char_long);

		/*** 读取FLSH ***/ 
		Read_Sector_Flash(0x000000, ReadData_char, char_long);   //1k = 1024byte (不是位)
		printf("\r\n Flash读取的数据为:\r\n%s\r\n", ReadData_char); 
...
/***************  写小数 **************************************/
		...
		Write_More_256Byte_YeHuo(0x000000, (void*)Float_WriteData, sizeof(Float_WriteData)); //写入>256字节
		//Write_Less256byte_Flash(0x000000, (void*)Float_WriteData, 20);   //注意float型数据的大小 4byte 因此写一个小数要发生四次
		/** 读Flash **/
        Read_Sector_Flash(0x000000, (void*)Float_ReadData, sizeof(Float_WriteData));
		printf("\r\n 读取的小数的数据为\r\n");
		for(j=0; j<20; j++) //打印向Flash写入值
		{
			printf("%f \r\n", Float_ReadData[j]); 
